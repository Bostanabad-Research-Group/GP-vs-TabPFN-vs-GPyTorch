...
def train_eval_gp(
    ...
    loss_type: str = "nll",  # "nll", "loo", "kf", or "nll_kf"
):
    """
    ...
    # Measure prediction time (actual evaluate only; setup below is not part of Pred)
    ...
    # Do NOT pin BLAS/torch threads during prediction. inner_max_num_threads is for
    # training-worker parity only; forcing 1 thread here inflated Pred ~5-10x vs old runs.
    ...
    best_iter_value = None
    jitter_value = None
    jitter_max_value = None

    if best_run is not None:
        # Prefer direct field from single-run trainer output
        if best_run.get("best_lbfgs_iter") is not None:
            best_iter_value = best_run.get("best_lbfgs_iter")
        ...
        for cb_key, stored_params_list in callback_data.items():
            if "FinalParameterStorage" in cb_key or "ParameterStorage" in cb_key:
                if stored_params_list:
                    ...
                    if record.get("best_iter") is not None:
                        best_iter_value = record.get("best_iter")
        ...
        gp_metric.update(
            {
                ...
                "best_iter": best_model_metrics.get("best_iter"),
            }
        )
        # Keep best_epoch only when best_iter is unavailable (non-LBFGS paths).
        if best_model_metrics.get("best_iter") is None:
            gp_metric["best_epoch"] = best_model_metrics.get("best_epoch")
