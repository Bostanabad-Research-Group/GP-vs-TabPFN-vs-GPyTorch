"""
Generate separate RRMSE / NIS / NCRPS IDETC violin PDFs (paper style).

Matches:
  results_paper/IDETC_figures_orig_ref/plots_gpplus_comparison        (no LOO)
  results_paper/IDETC_figures_orig_ref/plots_gpplus_comparison_LOO    (with LOO)

Default outputs (under results_IDETC/):
  plots_gpplus_comparison/       -> GP+, GP+(PE), PFN 2.5, PFN 2.0, GPyTorch
  plots_gpplus_comparison_LOO/   -> same + GP+(LOO)

For each problem/xdim, writes three PDFs:
  *_gpplus_comparison_RRMSE.pdf
  *_gpplus_comparison_NIS.pdf
  *_gpplus_comparison_NCRPS.pdf
...
Run from experiments/:
  ...
  python experimental_utils/IDETC_plot_gpplus_comparison.py
  python experimental_utils/IDETC_plot_gpplus_comparison.py --only no_loo
  python experimental_utils/IDETC_plot_gpplus_comparison.py --only loo
"""
...
def _plot_metric_axis(
    ...
    ax.set_xticklabels([DISPLAY_NAMES.get(m, m) for m in models], fontsize=22)
...
def make_separate_metric_plots(
    ...
    *,
    ...
    for (problem, xdim), prob_xdim_df in non_m2ax.groupby(["problem", "xdim"]):
        ...
        n_rows = len(dims)
        fig_width, fig_height = 12, 3 * n_rows
...
def _default_paths(root: Path) -> dict:
    return {
        "gp_dir": root / DIR_GP,
        "pe_dir": root / DIR_PE,
        "loo_dir": root / DIR_LOO,
        "pfn25_dir": root / DIR_PFN25,
        "pfn20_dir": root / DIR_PFN20,
        "gpytorch_dir": root / DIR_GPYTORCH,
        "out_no_loo": root / "plots_gpplus_comparison",
        "out_loo": root / "plots_gpplus_comparison_LOO",
    }


def collect_idetc_df(
    *,
    gp_dir: Path,
    pe_dir: Path,
...
def main() -> None:
    paths = _default_paths(DEFAULT_ROOT)
    parser = argparse.ArgumentParser(description="IDETC separate RRMSE/NIS/NCRPS violin PDFs.")
    parser.add_argument("--root", type=Path, default=DEFAULT_ROOT, help="results_IDETC root")
    ...
    out_no_loo = args.out_no_loo or (root / "plots_gpplus_comparison")
    out_loo = args.out_loo or (root / "plots_gpplus_comparison_LOO")
    problems = set(args.problems) if args.problems else None

    order_no_loo = ["gpplus", "GP+ (PE)", "tabpfn_v2.5", "tabpfn_v2", "gpytorch"]
    order_loo = ["gpplus", "GP+ (PE)", "GP+ (LOO)", "tabpfn_v2.5", "tabpfn_v2", "gpytorch"]
