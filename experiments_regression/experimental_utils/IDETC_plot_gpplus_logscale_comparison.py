...
Same format as IDETC_plot_gpplus_comparison.py (stacked N rows, per-metric PDFs),
but wider figures to fit original + log-scale GP variants.

Default data root: results_logscale_study/
  Original:   Gaussian, PE, LOO, PFN 2.5, PFN 2.0, GPyTorch (corrected LBFGS)
  Log-scale:  Gaussian (log), PE (log), LOO (log), GPyTorch (log, corrected LBFGS)

Default problems: zakharov, buckling

Outputs (under results_logscale_study/):
  plots_gpplus_logscale_comparison/       -> all models (incl. LOO + log variants)
  plots_gpplus_logscale_comparison_LOO/   -> same (legacy alias path)

Run from experiments/:

  python experimental_utils/IDETC_plot_gpplus_logscale_comparison.py
  python experimental_utils/IDETC_plot_gpplus_logscale_comparison.py --only no_loo
  python experimental_utils/IDETC_plot_gpplus_logscale_comparison.py --only loo
"""
...
def make_separate_metric_plots(
    ...
    *,
    ...
    for (problem, xdim), prob_xdim_df in non_m2ax.groupby(["problem", "xdim"]):
        ...
        do_legend = (
            show_legend
            and bool(legend_on_problem)
            and bool(noise_levels)
            and legend_on_problem.lower() in str(problem).lower()
        )
        ...
        for row_idx, dim in enumerate(dims):
            ...
            for metric in metrics:
                ...
                if do_legend and legend_elements is not None and row_idx == 0:
                    leg = ax.legend(
...
def collect_logscale_df(
    *,
    ...
    extras = [
        ...
        f"GPyTorch (log)={gpytorch_log_dir}",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--gpytorch_dir", type=Path, default=None)
    parser.add_argument("--gpytorch_log_dir", type=Path, default=None)
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Single output dir (used with --only core, or overrides --out_loo/--out_no_loo when set with those modes).",
    )
    parser.add_argument(
        "--only",
        choices=("both", "no_loo", "loo", "core"),
        default="both",
        help="Which plot sets to write (default: both). "
        "'core' = GP+, GP+(log), PFN 2.5, PFN 2.0, GPyTorch, GPyTorch (log).",
    )
    parser.add_argument(
        "--problems",
        nargs="*",
    ...
    parser.add_argument("--no_legend", action="store_true", help="Omit noise/mean/median legends.")
    ...
    # Pair each GP+ / GPyTorch variant with its log-scale counterpart.
    order_no_loo = [
        "gpplus",
        "GP+ (log)",
        "GP+ (PE)",
    ...
    if args.only == "core":
        out_core = args.out or (root / "plots_gpplus_logscale_comparison_core")
        df = collect_logscale_df(
            ...
            gpytorch_log_dir=gpytorch_log,
            include_loo=False,
            problems=problems,
            noise_levels=args.noise_levels,
        )
        if df.empty:
            raise SystemExit("No data for core plots.")
        print(f"[core] rows={len(df)} models={sorted(df['model_family'].unique())}")
        print(f"[core] problems={sorted(df['problem'].unique())}")
        make_separate_metric_plots(
            df,
            out_core,
            order_core,
    ...
    if args.only in ("both", "no_loo"):
        out_dir = args.out or out_no_loo
        df = collect_logscale_df(
            gp_dir=gp_dir,
            pe_dir=pe_dir,
            loo_dir=None,
        ...
        make_separate_metric_plots(
            df, out_dir, order_no_loo, remove_outliers=args.remove_outliers, show_legend=not args.no_legend
        )

    if args.only in ("both", "loo"):
        out_dir = args.out or out_loo
        df = collect_logscale_df(
            gp_dir=gp_dir,
            pe_dir=pe_dir,
            loo_dir=loo_dir,
        ...
        make_separate_metric_plots(
            df, out_dir, order_loo, remove_outliers=args.remove_outliers, show_legend=not args.no_legend
        )
