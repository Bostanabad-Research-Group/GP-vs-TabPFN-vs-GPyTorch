...
date = "10_runs_logging_full_Gaussian_2k"
...
opt = defaults.TRAINER_OPTIMIZER_CLASS
opt_kwargs = defaults.TRAINER_OPTIMIZER_KWARGS
print(
    f"[B2 Gaussian 2k] optimizer={opt.__name__}, kwargs={opt_kwargs}, "
    f"epochs={defaults.TRAINER_NUM_EPOCHS} (one LBFGS step per init), "
    f"num_inits={num_inits}, num_runs={num_runs}, save=./{folder}/{date}/"
)
assert opt.__name__ == "LBFGSScipy", f"Expected LBFGSScipy, got {opt}"
assert defaults.TRAINER_NUM_EPOCHS == 1
...
# # # # %% Buckling ------------------------------------------------------------------------------------------------
# Keep default MVMF (do NOT pass kernel_type — that replaces MVMF with plain SF Gaussian)
