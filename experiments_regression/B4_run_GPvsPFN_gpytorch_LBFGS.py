"""Run the full IDETC gpytorch suite with LBFGSScipy (1 epoch × max_iter=2000).

Matches the problem/setup matrix in results_IDETC/10_runs_gpytorch_orig.
Saves to results_IDETC/10_runs_gpytorch_corrected_LBFGS_2k.
"""
...
# Patch before those imports so LBFGS settings apply.
import defaults_gpytorch_LBFGS as defaults_gpytorch
...
def main() -> None:
    ...
    date = "10_runs_gpytorch_corrected_LBFGS_2k"
    ...
    opt = defaults_gpytorch.TRAINER_OPTIMIZER_CLASS
    opt_kwargs = defaults_gpytorch.TRAINER_OPTIMIZER_KWARGS

    print(
        f"[B4 LBFGS 2k] optimizer={opt.__name__}, "
        f"kwargs={opt_kwargs}, "
        f"epochs={defaults_gpytorch.TRAINER_NUM_EPOCHS} (one LBFGS step per init), "
        f"num_inits={num_inits}, num_runs={num_runs}, save=./{folder}/{date}/"
    )
    assert opt.__name__ == "LBFGSScipy", f"Expected LBFGSScipy, got {opt}"
    assert defaults_gpytorch.TRAINER_NUM_EPOCHS == 1
    assert opt_kwargs.get("max_iter") == 2000
    assert opt_kwargs.get("max_eval") == 5000
    ...
    print(f"[B4 LBFGS 2k] done. Results in ./{folder}/{date}/")
