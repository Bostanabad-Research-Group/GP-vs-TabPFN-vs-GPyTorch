"""Run buckling + zakharov gpytorch log-scale suite with LBFGS defaults.

Saves to results_logscale_study/10_runs_gpytorch_corrected_LBFGS_logscale.
"""
...
def main() -> None:
    ...
    date = "10_runs_gpytorch_corrected_LBFGS_logscale"
    ...
    print(
        f"[B4 LBFGS logscale] optimizer={defaults_gpytorch.TRAINER_OPTIMIZER_CLASS.__name__}, "
        f"lr={defaults_gpytorch.TRAINER_LR} (LBFGS_LR={defaults_gpytorch.LBFGS_LR}), "
        f"epochs={defaults_gpytorch.TRAINER_NUM_EPOCHS}, "
    ...
    print(f"[B4 LBFGS logscale] done. Results in ./{folder}/{date}/")
