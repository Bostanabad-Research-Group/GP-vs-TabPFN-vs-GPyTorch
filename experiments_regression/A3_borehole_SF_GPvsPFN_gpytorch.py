...
def borehole_SF_GPvsPFN(num_runs=defaults.NUM_RUNS,
    ...
    # Match GP+ pool: max(num_runs, 20) disjoint folds (8 borehole inputs)
    train_per_run = train_size * 8
    ...
    # Same fold indices as GP+ counterpart
    ...
    print("="*60)
    opt_name = optimizer_class.__name__ if optimizer_class is not None else "default"
    print(f"Trainer details (GPyTorch {opt_name}): \n\tnumber of epochs: {num_epochs}\n\tlearning rate: {lr}\n\toptimizer: {opt_name}\n\tdevice: {gp_device}\n\tcont_cols: {cont_cols}\n\tcat_cols: {cat_cols}\n\tsource_cols: {source_cols}\n\tqual_dict: {qual_dict}\n\tX_standardize: {standardize_X}\n\ty_standardize: {standardize_y}")
