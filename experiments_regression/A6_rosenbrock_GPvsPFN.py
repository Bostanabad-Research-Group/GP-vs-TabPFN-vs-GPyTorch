...
def rosenbrock_GPvsPFN(num_runs=defaults.NUM_RUNS,
    ...
    for i in range(num_runs):
        ...
        if run_models in [None, 'gp']:
            ...
            # Create GP model (kernel_mod from kernel_type, else defaults.SF_kernel)
...
if __name__ == "__main__":
    # PE Rosenbrock 80D rerun — no trainer/LBFGS logging.
    _PE_COMMON = dict(
        dimensions=80,
        num_test=5000,
        num_runs=10,
        ...
        trainer_info=False,
        log_lbfgs_inner=False,
        save_path="./IDETC_results_new/10_runs_logging_full_PE/rosenbrock",
    )
    # (5, 0.002) already finished; rerun the rest.
    for train_mult, noise in ((5, 0.002), (20, 0.002), (5, 0.08), (20, 0.08)):
        print("\n" + "=" * 80)
        print(f"START Rosenbrock PE Dx=80 Dn={train_mult} noise={noise}")
        print("=" * 80)
        rosenbrock_GPvsPFN(
            **_PE_COMMON,
        ...
        print(f"DONE Rosenbrock PE Dx=80 Dn={train_mult} noise={noise}")
